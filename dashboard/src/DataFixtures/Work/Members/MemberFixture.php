...
class MemberFixture extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $this->addReference(self::REFERENCE_ADMIN, $member);

        $member = $this->createMember($user, $customers);
        $manager->persist($member);
        $this->addReference(self::REFERENCE_USER, $member);
    ...
    private function createMember(User $user, Group $group): Member
    {
        return new Member(
            ...
            new Email($user->getEmail() ? $user->getEmail()->getValue() : 'official@app.test')
        );
    }
}
